...
function completedHistory(r){
 ...
 return `<section class="v823History"><h2>COMPLETED GROUPS</h2>${Object.keys(by).map(st=>`<details ${Number(st)===r.v8Groups.stage?'open':''}><summary>${stageLabel(r.v8Groups,Number(st))}<span>${by[st].length} COMPLETE</span></summary>${by[st].map(grp=>`<button type="button" onclick="rhV8GroupReveal('${r.id}','${grp.id}')"><b>GROUP ${grp.index}</b><span>${(grp.standings||[]).slice(0,2).map(x=>esc(carName(carById(x.id)))).join(' • ')||'View standings'}</span><strong>VIEW ›</strong></button>`).join('')}</details>`).join('')}</section>`;
}

/* RaceHub v5.7.95 — isolated Final Standings page */
(()=>{
  const VERSION='5.7.95';
  ...
  function closeFinal(){
    ...
    document.querySelectorAll('body > [data-rh595-hidden]').forEach(el=>{
      const old=el.getAttribute('data-rh595-display');
  ...
  window.rhShowFinalStandingsV5793=function(runId){
    ...
    // Hard-remove every legacy app surface from layout before mounting Final Standings.
    [...document.body.children].forEach(el=>{
      if(el.tagName==='SCRIPT') return;
      el.setAttribute('data-rh595-hidden','1');
      el.setAttribute('data-rh595-display',el.style.display||'');
      el.setAttribute('aria-hidden','true');
      el.style.setProperty('display','none','important');
    });
    document.body.appendChild(root);
    document.body.classList.add('rh593-final-open');
    document.documentElement.classList.add('rh593-final-open');
    document.body.style.setProperty('overflow','hidden','important');
    document.documentElement.style.setProperty('overflow','hidden','important');
    root.style.cssText='position:fixed!important;inset:0!important;z-index:2147483647!important;display:block!important;opacity:1!important;visibility:visible!important;overflow-y:auto!important;background:#01070b!important;';
    ...
    const resetTop=()=>{root.scrollTop=0;document.scrollingElement&&(document.scrollingElement.scrollTop=0);window.scrollTo(0,0)};
    resetTop();
    requestAnimationFrame(()=>{resetTop();root.querySelector('.rh593-final')?.classList.add('live');requestAnimationFrame(resetTop)});
  };
  ...
  // Final safeguard: every completed-championship leaderboard action routes here.
  document.addEventListener('click',e=>{
    ...
    const id=m?.[1] || runs().find(r=>r.status==='complete')?.id;
    if(id){e.preventDefault();e.stopImmediatePropagation();window.rhShowFinalStandingsV5793(id)}
  },true);

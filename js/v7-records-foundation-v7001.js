/* OTG! v7.0.1 — Phase A Records foundation: Track Records + Car History. */
(()=>{
'use strict';
...
function add(out,res,source,kind,round){
  ...
  out.push({track,layout,k:key(track,layout),time:Number(res.time),carId:String(res.carId||''),date:res.date||source?.completedAt||source?.updatedAt||'',kind,tag:typeTag(kind,source),sourceName:String(source?.name||source?.title||'').trim(),sourceStatus:String(source?.status||''),sourceId:String(source?.id||'')});
}
function allRows(){

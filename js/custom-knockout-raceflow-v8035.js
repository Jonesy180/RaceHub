/* OTG! v8.0.35 — Custom Knockout race flow. */
(()=>{
'use strict';
...
window.rhCustomKOBack8035=koBack;
...
function renderSetup(e,r){show('event');const ko=e.customKO;document.getElementById('event').innerHTML=`<div class="rhContent"><button class="v8GroupVisibleBack" onclick="rhCustomKOBack8035()">‹ BACK</button><section class="rhSection"><small>CUSTOM KNOCKOUT</small><h2>${escx(r.label)} SETUP</h2><p>${r.matchCount} match${r.matchCount===1?'':'es'}${r.straightThroughIds?.length?` • ${r.straightThroughIds.length} straight through`:''}. Choose one track for this round.</p></section><section class="rhSection"><h2>Round Track</h2><input id="rhKOTrack8035" value="${escx(r.trackName||'')}" placeholder="Track / race name"><button class="btn" onclick="rhCustomKOStartRound8035('${e.id}')">START ${escx(r.label)}</button></section></div>`;window.scrollTo(0,0)}

window.rhCustomKOStartRound8035=function(id){const e=event(id),r=activeRound(e);if(!e||!r)return;const track=String(document.getElementById('rhKOTrack8035')?.value||'').trim();if(!track)return toast('Choose a track first');r.trackName=track;let ids;

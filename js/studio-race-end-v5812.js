/* RaceHub v5.8.12 — authoritative race-end flow (Result Summary + Final Standings). */
(()=>{
  'use strict';
  ...
  function finalHtml(kind,item,rows,currentId){
    ...
    return `<div class="rhFinal5812">
      <header class="rhFinal5812Head"><button id="rhFinalTopBack" aria-label="Back to ${kind==='event'?'events':'championships'}">‹</button><div><h1>FINAL STANDINGS</h1><p>${esc(item.name)}</p></div></header>
      <section class="rhFinal5812Board">
        <div class="rhFinal5812Title"><span>◆◆</span><h2>OFFICIAL RESULTS</h2><span>◆◆</span></div>
        <div class="rhFinal5812Cols"><span>POS</span><span>CAR</span><span>TOTAL TIME</span><span>GAP</span></div>
        <div class="rhFinal5812Scroll" id="rhFinalScroll">${rows.map((x,i)=>`<div class="rhFinal5812Row ${String(x.id)===String(currentId)?'current':''}"><i>${String(i+1).padStart(2,'0')}</i><span>${esc(getName(x.id))}</span><strong>${fmt(x.total)}</strong><em>${i===0?'—':'+'+fmt(x.total-leader)}</em></div>`).join('')}</div>
        ${rows.length>5?'<div class="rhFinal5812More">⌄ &nbsp; SCROLL FOR MORE RESULTS &nbsp; ⌄</div>':''}
      </section>
      <section class="rhFinal5812Winner">${kind==='event'?'<div class="rhFinal5812EventTrophy">🏁</div>':`<img src="${trophy(item)}" alt="">`}<div><small>${championLabel}</small><b>${esc(getName(winner?.id))}</b><span>TOTAL TIME</span><strong>${winner?fmt(winner.total):'—'}</strong></div></section>
      <div class="rhFinal5812Actions"><button class="secondary" id="rhFinalSecondary">${kind==='event'?'BACK TO EVENTS':'VIEW HALL OF FAME'}</button><button class="primary" id="rhFinalBack">${kind==='event'?'BACK TO EVENTS':'BACK TO CHAMPIONSHIPS'}</button></div>
    </div>`;
  }
  function mountFinal(kind,item,rows,currentId){
    ...
    document.body.classList.add('rhFinalActive5812');
    host.innerHTML=finalHtml(kind,item,rows,currentId);
    const back=()=>{document.body.classList.remove('rhFinalActive5812');if(kind==='event'){rhRenderEvents();show('events')}else{rhRenderFestival();show('festival')}window.scrollTo(0,0)};
    byId('rhFinalBack')?.addEventListener('click',back);
    byId('rhFinalTopBack')?.addEventListener('click',back);
    byId('rhFinalSecondary')?.addEventListener('click',()=>{if(kind==='event')back();else{document.body.classList.remove('rhFinalActive5812');window.rhRecordsMode='hall';rhRenderRecords();show('hall')}});
  ...
  window.rhShowFinalStandingsV5812=showFinalRun;
  window.rhShowEventFinalStandingsV5812=showFinalEvent;

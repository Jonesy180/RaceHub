/* RaceHub v5.8.33 — Championship in Progress full rebuild */
(function(){
  'use strict';
  ...
  window.rhShowCurrentStandingsV5833=function(id){
    ...
    $('festival').innerHTML=`<div class="rhStandings33">
      <header><button onclick="rhOpenRun('${safe(id)}')" aria-label="Back">‹</button><div><small>CURRENT STANDINGS</small><h1>${safe(r.name)}</h1></div></header>
  ...
  window.rhOpenRun=function(id){
    ...
    $('festival').innerHTML=`<div class="rhChamp33">
      <header class="rhChamp33Hero">
        <div class="rhChamp33HeroShade"></div>
        <div class="rhChamp33Title"><small>${competitionLabel(r)}</small><h1>${safe(r.name)}</h1>${roundName?`<p>${safe(roundName)}</p>`:''}</div>
      </header>
      <main class="rhChamp33Body">
        <section class="rhChamp33Progress">
          <div class="rhChamp33ProgressTop"><div><small>CHAMPIONSHIP PROGRESS</small><b>${cp.complete} OF ${cp.total} CARS COMPLETE</b></div><strong>${cp.pct}%</strong></div>
          <div class="rhChamp33Bar"><i style="width:${p.pct}%"></i></div>
          <div class="rhChamp33Stats"><span><b>${entries.length}</b><small>FROZEN CARS</small></span><span><b>${rounds.length}</b><small>${rounds.length===1?'ROUND':'ROUNDS'}</small></span><span><b>${cp.complete}</b><small>CARS COMPLETE</small></span></div>
        </section>
        ...
        <section class="rhChamp33Next">
          ...
          <div class="rhChamp33NextRows">
            ...
            <div class="status"><i>3</i><b>${remainingAfterPreview} CARS REMAINING</b></div>
          </div>
        </section>
        <footer class="rhChamp33Footer">
          <button onclick="rhShowCurrentStandingsV5833('${safe(r.id)}')"><i>🏆</i><b>VIEW CURRENT<br>STANDINGS</b></button>

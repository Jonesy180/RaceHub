/* OTG! v8.0.65 — manifest updater + direct-load Safety Backup gate. */
(()=>{
'use strict';
const CURRENT=()=>document.querySelector('meta[name="racehub-version"]')?.content||'8.0.65';
...
function createAndVerifyAllSpaceSafetyBackups(){
 try{
  ...
  for(const live of state.spaces){
   ...
   const canonicalManual=clone(stored.backups||[]).filter(b=>b&&b.type!=='safety'),manualDigest=JSON.stringify(canonicalManual);
   live.backups=clone(canonicalManual);
   const b=window.rhBackupSnapshotV8012(live,'safety');live.safetyBackup=b;
  ...
  for(const e of expected){const stored=verifyState.spaces.find(x=>String(x.id)===String(e.spaceId)),b=stored?.safetyBackup;if(!b||b.id!==e.id||b.type!=='safety'||window.rhSafetyDigestV8013(b)!==e.digest)return{ok:false,reason:'verify-failed'};const manuals=(stored.backups||[]).filter(x=>x&&x.type!=='safety');if(JSON.stringify(manuals)!==e.manualDigest)return{ok:false,reason:'manual-backups-changed'};}
...
async function install(m,d){
 ...
 const gate=createAndVerifyAllSpaceSafetyBackups();if(!gate.ok){out.textContent='UPDATE COULDN’T START — Safety Backup could not be verified. Your data has not been changed.';btn.disabled=false;return;}
...
window.rhStartupUpdateGate8062=async function(){
 ...
 try{const m=await latest();const target=cmp(m.version,CURRENT())>=0?m:{version:CURRENT(),worker:'./otg-service-worker-v8065.js',message:'OTG! v8.0.65 is ready.'};showUpdate(target,{direct:true});return true;}catch(_){showUpdate({version:CURRENT(),worker:'./otg-service-worker-v8065.js',message:'OTG! v8.0.65 is ready.'},{direct:true});return true;}
};
...
window.rhCreateAndVerifyAllSpaceSafetyBackupsV8065=createAndVerifyAllSpaceSafetyBackups;
})();

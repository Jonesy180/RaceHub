...
window.rhDeleteBackupConfirm=function(id){const b=(rhSpace().backups||[]).find(x=>x.id===id);if(!b)return;rhConfirm({title:'DELETE THIS BACKUP?',copy:'This permanently deletes only the selected saved backup.',detail:`${b.spaceName} • ${new Date(b.date).toLocaleString('en-GB')}`,safeguard:'Your current RaceHub Space, its racing data and all other Spaces will not be affected.',confirmLabel:'DELETE BACKUP',danger:true,onConfirm:`rhDeleteBackupFinal('${id}')`})};
...
window.rhAbout=function(){
 ...
 <section class="rhSection"><h2>RACEHUB INFORMATION</h2><div class="rhInfoRow"><span>App Version</span><b>v5.8.35</b></div><div class="rhInfoRow"><span>Data / Backup Version</span><b>v1.0.0</b></div><div class="rhInfoRow"><span>Current RaceHub Space</span><b>${safe(s.name)}</b></div><div class="rhInfoRow"><span>Driver Profile (Global)</span><b>${safe(state.driverName||'Driver')}</b></div></section>
...
<section class="rhSection rhSettingPanel"><button class="rhSettingRow" onclick="rhAbout()"><b>ABOUT RACEHUB</b><span>RaceHub v5.7.80 • ›</span></button></section></div>`};

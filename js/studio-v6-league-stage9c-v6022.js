/* RaceHub v6.0.22 — Stage 9C League Results + Review/Confirmation — digital timing correction.
...
(function(){
  ...
  const segDigit=ch=>`<i class="rhSegDigit">${'abcdefg'.split('').map(x=>`<span class="s${x} ${(SEG[ch]||'').includes(x)?'on':''}"></span>`).join('')}</i>`;
  const segGroup=(v,n)=>String(v||'').padStart(n,'0').slice(-n).split('').map(segDigit).join('');
  function splitLeagueTime(v){const m=String(v||'').match(/^(\d{1,2}):([0-5]?\d)\.(\d{1,3})$/);return m?{Min:m[1],Sec:m[2],Ms:m[3].padEnd(3,'0')}:{Min:'',Sec:'',Ms:''};}
  function leagueTimePart(i,part,len,value,label){const id=`rhLTime21_${i}_${part}`;return `<label class="rhLeagueTimePartV6022 ${part==='Ms'?'ms':''}"><span id="${id}Display" class="rhLeagueSegDisplayV6022" aria-hidden="true">${segGroup(value||'0',len)}</span><input id="${id}" type="tel" inputmode="numeric" pattern="[0-9]*" maxlength="${len}" autocomplete="off" value="${safe(value)}" aria-label="${label}" data-league-part="${part}" onfocus="this.select()" oninput="rhLeagueTimeInputV6022(this,${len})"></label>`;}
  function leagueDigitalTime(i,timeText){const t=splitLeagueTime(timeText);return `<div class="rhLeagueDigitalTimeV6022" aria-label="Race time">${leagueTimePart(i,'Min',2,t.Min,'minutes')}<i>:</i>${leagueTimePart(i,'Sec',2,t.Sec,'seconds')}<i>.</i>${leagueTimePart(i,'Ms',3,t.Ms,'milliseconds')}</div>`;}
  function readLeagueTimeParts(i){const g=p=>String(q(`rhLTime21_${i}_${p}`)?.value||'').trim();const a=g('Min'),b=g('Sec'),c=g('Ms');if(!a&&!b&&!c)return '';return `${a}:${b}.${c}`;}
  function refreshLeagueSegment(input){const part=input.dataset.leaguePart,max=part==='Sec'?59:part==='Ms'?999:99,len=input.maxLength||2;let raw=String(input.value||'').replace(/\D/g,'').slice(0,len);if(raw&&Number(raw)>max)raw=String(max);input.value=raw;const d=q(input.id+'Display');if(d)d.innerHTML=segGroup(raw||'0',len);}
  window.rhLeagueTimeInputV6022=function(input,len){refreshLeagueSegment(input);if(input.value.length>=len){const part=input.dataset.leaguePart,row=input.closest('.rhLeagueDigitalTimeV6022');const next=part==='Min'?row?.querySelector('[data-league-part="Sec"]'):part==='Sec'?row?.querySelector('[data-league-part="Ms"]'):null;if(next){next.focus();next.select?.();}}};
  ...
  function syncDraft(lid,eid){
    ...
    d.rows.forEach((r,i)=>{r.position=String(q(`rhLPos21_${i}`)?.value||'').trim();r.timeText=readLeagueTimeParts(i);});
  }

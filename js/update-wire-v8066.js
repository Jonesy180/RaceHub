/* OTG! v8.0.66 — manifest updater + IndexedDB Safety Backup gate. */
(()=>{
'use strict';
const CURRENT=()=>document.querySelector('meta[name="racehub-version"]')?.content||'8.0.66';
...
window.rhStartupUpdateGate8062=async function(){
 ...
 try{const m=await latest();const target=cmp(m.version,CURRENT())>=0?m:{version:CURRENT(),worker:'./otg-service-worker-v8066.js',message:'OTG! v8.0.66 is ready.'};showUpdate(target,{direct:true});return true;}catch(_){showUpdate({version:CURRENT(),worker:'./otg-service-worker-v8066.js',message:'OTG! v8.0.66 is ready.'},{direct:true});return true;}
};

/* OTG! v8.0.20 — manifest-led, Safety-Backup-gated updater */
(()=>{
'use strict';
const CURRENT=()=>document.querySelector('meta[name="racehub-version"]')?.content||'8.0.20';
...
function createAndVerifyAllSpaceSafetyBackups(){
 try{
  // IMPORTANT: mutate OTG!'s authoritative in-memory state, then save through rhSave().
  // Direct localStorage writes can be overwritten by the still-live in-memory state during update/reload.
  if(typeof state==='undefined'||!state||!Array.isArray(state.spaces)||!state.spaces.length)return{ok:false,reason:'no-spaces'};
  if(typeof window.rhBackupSnapshotV8012!=='function'||typeof rhSave!=='function'||typeof window.rhVerifySafetyBackupV8013!=='function')return{ok:false,reason:'backup-api-unavailable'};
  const expected=[];
  for(const s of state.spaces){
   const beforeManual=JSON.stringify(s.backups||[]);
   const b=window.rhBackupSnapshotV8012(s,'safety');
   s.safetyBackup=b;
   // Manual slots are user-owned and must remain byte-for-byte untouched.
   if(JSON.stringify(s.backups||[])!==beforeManual)return{ok:false,reason:'manual-backups-mutated'};
   expected.push({spaceId:s.id,id:b.id,digest:window.rhSafetyDigestV8013(b),manualDigest:beforeManual});
  }
  rhSave();
  for(const e of expected){
   const persisted=window.rhPersistedSpaceV8013?.(e.spaceId);
   const b=persisted?.safetyBackup;
   if(!b||b.id!==e.id||b.type!=='safety'||window.rhSafetyDigestV8013(b)!==e.digest)return{ok:false,reason:'verify-failed'};
   if(JSON.stringify(persisted.backups||[])!==e.manualDigest)return{ok:false,reason:'manual-backups-changed'};
  }
...
window.rhAutoCheckForUpdateV8020=()=>check(false);
})();

/* OTG! v8.0.61 — authoritative race-end flow with per-round/course Result Summary average comparison. */
(()=>{
  'use strict';
  const VERSION='8.0.61';
  ...
  function roundCourseAverage(owner,res){
    ...
    const prior=(owner?.results||[]).filter(x=>{
      if(String(x.id)===String(res?.id)||!Number.isFinite(Number(x.time))||Number(x.time)<=0)return false;
  ...
  function runSummary(run,res){
    ...
    mountSummary('festival',{roundName:res.roundName,title:typeLabel(run.type||run.championshipType),carLine:`${getName(res.carId)} • ${fmt(res.time)}`,rows,carId:res.carId,average:roundCourseAverage(run,res),label,sub,back:viewStandings,next});
  }
  function eventSummary(event,res){
    ...
    mountSummary('event',{roundName:res.roundName,title:event.name,carLine:`${getName(res.carId)} • ${fmt(res.time)}`,rows:partialEventRows(event),carId:res.carId,average:roundCourseAverage(event,res),label:done?'FINAL STANDINGS':'CONTINUE EVENT',sub:done?'VIEW OFFICIAL CLASSIFICATION':'RETURN TO EVENT',back:viewStandings,next});
  }

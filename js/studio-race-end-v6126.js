/* OTG! v6.0.126 — authoritative race-end flow + locked no-Hubs record celebration. */
(()=>{
  'use strict';
  ...
  const SEG6126={0:'abcdef',1:'bc',2:'abdeg',3:'abcdg',4:'bcfg',5:'acdfg',6:'acdefg',7:'abc',8:'abcdefg',9:'abcdfg'};
  function segDigit6126(ch){return `<i class="rhGoldSegDigitV6126">${'abcdefg'.split('').map(x=>`<span class="s${x} ${(SEG6126[ch]||'').includes(x)?'on':''}"></span>`).join('')}</i>`;}
  function digitalTime6126(value){
    const text=fmt(value),m=text.match(/(\d{1,2}):(\d{2})\.(\d{3})/);
    if(!m)return `<strong class="rhRecordDigitalFallbackV6126">${esc(text)}</strong>`;
    const group=v=>String(v).split('').map(segDigit6126).join('');
    return `<div class="rhRecordDigitalV6126" aria-label="${esc(text)}"><span class="grp">${group(m[1].padStart(2,'0'))}</span><b>:</b><span class="grp">${group(m[2])}</span><b>.</b><span class="grp ms">${group(m[3])}</span></div>`;
  }
  function recordMedals6126(res,kind){
    const localLabel=kind==='event'?'EVENT RECORD':'CHAMPIONSHIP RECORD';
    return `<div class="rhRecordMedalsV6126">${res.championshipRecord?`<article class="silver"><span>★</span><b>${localLabel}</b></article>`:'<i></i>'}${res.allTime?`<article class="gold"><span>★</span><b>ALL-TIME OTG!<br>RECORD</b></article>`:'<i></i>'}</div>`;
  }
  function showHubs(kind,owner,res,viewStandings,continueAction){
    ...
    host.innerHTML=`<div class="rhRecordPageV6126">
      <button class="rhRecordBackV6126" id="rhHubs29Back">BACK</button>
      <main class="rhRecordMainV6126">
        <header><small>${res.championshipRecord?`NEW ${localLabel}!`:'NEW ALL-TIME RECORD!'}</small><h1>${headline}</h1></header>
        <section class="rhRecordHeroV6126">${recordMedals6126(res,kind)}<img src="assets/final/record-celebration-car-podium-v6126.png?v=6126" alt="Neutral OTG! record celebration car on winner podium"></section>
        <section class="rhRecordBestV6126"><small>YOUR NEW BEST TIME</small>${digitalTime6126(res.time)}</section>
        ${previous!=null?`<section class="rhRecordStatV6126 previous"><span class="icon">◷</span><div><small>PREVIOUS BEST</small><b>${fmt(previous)}</b></div></section><section class="rhRecordStatV6126 improvement"><span class="icon">↗</span><div><small>IMPROVEMENT</small><b>−${fmt(improvement)}</b></div></section>`:''}
      </main>
      <footer class="rhRecordActionsV6126"><button id="rhHubs29Standings"><span>🏆</span><b>VIEW CURRENT STANDINGS</b></button><button id="rhHubs29Continue"><span>🏁</span><b>${owner.status==='complete'?'FINAL STANDINGS':kind==='event'?'CONTINUE EVENT':'CONTINUE CHAMPIONSHIP'}</b></button></footer>
    </div>`;

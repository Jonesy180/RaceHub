/* OTG! v8.0.51 — app-wide Smart Bar; click-safe for dynamically re-rendered Round 2+ fields. */
(()=>{
'use strict';
...
function refresh(el){const box=el._rhGlobalSmartBar;if(!box)return;const hit=best(el);if(!hit)return hide(el);box.innerHTML=`<button type="button" class="rhSmartSuggestionSingle"><span></span></button>`;box.querySelector('span').textContent=hit;box.hidden=false;box.querySelector('button').onclick=e=>{e.preventDefault();e.stopPropagation();el.value=hit;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));hide(el);requestAnimationFrame(()=>el.isConnected&&el.focus())}}

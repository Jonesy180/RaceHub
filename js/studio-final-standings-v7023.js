...
function leave(kind,host,item){document.body.classList.remove('rhFS28Active');host.classList.add('hidden');host.innerHTML='';if(kind==='event'){rhRenderEvents();show('events')}else if(item?.pickMyDrive||item?.value==='pick-my-drive'){window.rhPickMyDriveComingSoon?.()}else{rhRenderFestival();show('festival')}window.scrollTo(0,0)}
...
function mount(kind,item,rows){
 ...
 window.scrollTo(0,0);
 q('rhFS28Back')?.addEventListener('click',()=>leave(kind,host,item));
 q('rhFS28TopBack')?.addEventListener('click',()=>leave(kind,host,item));
 q('rhFS28Hall')?.addEventListener('click',()=>{if(kind==='event'){leave(kind,host,item);return}document.body.classList.remove('rhFS28Active');host.classList.add('hidden');host.innerHTML='';window.rhRecordsMode='hall';rhRenderRecords();show('hall');window.scrollTo(0,0)});
}

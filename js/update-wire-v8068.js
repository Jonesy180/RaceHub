/* OTG! v8.0.68 — manifest updater + IndexedDB Safety Backup gate. */
(()=>{
'use strict';
const CURRENT=()=>document.querySelector('meta[name="racehub-version"]')?.content||'8.0.68';
...
window.rhStartupUpdateGate8062=async function(){
 ...
 try{const m=await latest();const target=cmp(m.version,CURRENT())>=0?m:{version:CURRENT(),worker:'./otg-service-worker-v8068.js',message:'OTG! v8.0.68 is ready.'};showUpdate(target,{direct:true});return true;}catch(_){showUpdate({version:CURRENT(),worker:'./otg-service-worker-v8068.js',message:'OTG! v8.0.68 is ready.'},{direct:true});return true;}
};

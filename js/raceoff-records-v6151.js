/* OTG! v6.0.151 — Race Off records bridge.
...
function showRecord(flags,ro,rd,res,side,i,matchId){
  ...
  document.body.insertAdjacentHTML('beforeend',`<div id="rhRaceOffRecordV6151" class="rhOverlay rhRORecordOverlayV6151"><section class="rhRORecordModalV6151"><header><small>HUBS HAS SOME NEWS</small><h2>NEW RACE OFF RECORD</h2></header><img src="assets/final/hubs-pit-chief-v5829.png?v=5829" alt="Hubs, OTG! Pit Chief"><div class="rhRORecordCardsV6151">${flags.raceOffRecord?card('RACE OFF RECORD'):''}${flags.trackRecord?card('TRACK RECORD','gold'):''}${flags.personalBest?card('YOUR PB','cyan'):''}</div><div class="rhRORecordTimeV6151"><small>YOUR NEW BEST TIME</small><strong>${fmt(res.time)}</strong>${prev!=null?`<small>PREVIOUS BEST</small><b>${fmt(prev)}</b>${improvement>0?`<em>−${fmt(improvement)}</em>`:''}`:''}</div><blockquote>“${E(quote(flags))}”<cite>— HUBS • PIT CHIEF</cite></blockquote><button class="btn" id="rhRORecordContinueV6151">CONTINUE RACE OFF</button></section></div>`);

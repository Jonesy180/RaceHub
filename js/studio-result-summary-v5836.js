/* RaceHub v5.8.36 — podium Result Summary with locked average comparison */
(()=>{
  ...
  function averageTile(hist){
    if(!hist)return `<section class="rhPodiumAverageV5836 neutral"><div><small>CHAMPIONSHIP AVERAGE</small><b>—</b><span>No prior results</span></div><div><small>YOUR DIFFERENCE</small><b>—</b><span>First recorded result</span></div></section>`;
    ...
    return `<section class="rhPodiumAverageV5836 ${state}"><div><small>CHAMPIONSHIP AVERAGE</small><b>${fmt(hist.avg)}</b><span>Before this result • ${hist.count} result${hist.count===1?'':'s'}</span></div><div><small>YOUR DIFFERENCE</small><b>${sign}${fmt(Math.abs(hist.diff))}</b><span>${label}</span></div></section>`;
  }
